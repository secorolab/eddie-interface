...
void PID::set_gains(double p_gain, double i_gain, double d_gain, double decay_rate) {
    kp               = p_gain;
    ki               = i_gain;
    kd               = d_gain;
    this->decay_rate = decay_rate;
...
EddieRosInterface::EddieRosInterface(const rclcpp::NodeOptions &options)
    ...
    jnt_to_jac_dot_solver_leftarm = std::make_unique<KDL::ChainJntToJacDotSolver>(leftarm_chain);
    ik_solver_vel_leftarm         = std::make_unique<KDL::ChainIkSolverVel_pinv>(leftarm_chain);
    ...
    jnt_to_jac_dot_solver_rightarm = std::make_unique<KDL::ChainJntToJacDotSolver>(rightarm_chain);
    ik_solver_vel_rightarm         = std::make_unique<KDL::ChainIkSolverVel_pinv>(rightarm_chain);
...
void EddieRosInterface::compute_gravity_comp(events *eventData, EddieState *eddie_state) {
    ...
    for (int i = 0; i < num_jnts_rightarm; i++) {
        saturate(&tau_ctrl_rightarm(i), -30.0, 30.0);
